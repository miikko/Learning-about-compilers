...
unless($just_unpack) {
	...
	if($verbose);
    system($grading_cmd);
    # 21.07.2021 Added so that files in the 'grading' folder can be read and written to.
    # A problem with permissions arose when trying to read these files outside the Docker container where they were created.
    system("chmod -R 777 .");
}
